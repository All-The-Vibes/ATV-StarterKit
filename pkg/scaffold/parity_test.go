...
func TestDogfoodTemplateParity(t *testing.T) {
	...
	templateOnly := map[string]bool{
		...
		"atv-security": true,
		// atv-doctor ships only as a template (maintenance skill that
		// diagnoses ATV install health on consumer projects).
		"atv-doctor": true,
		// atv-update ships only as a template (maintenance skill that
		// updates marketplace plugins on consumer projects).
		"atv-update": true,
	}
