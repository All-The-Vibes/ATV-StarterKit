...
- **`atv-pack-maintenance` marketplace plugin** — bundles `/atv-doctor` and `/atv-update`. Install the flagship bundle with `copilot plugin install atv-starter-kit@atv-starter-kit` to get both skills (the granular `atv-pack-maintenance` entry is defined in `.github/plugin/marketplace.json` for reference but is not individually installable via the Copilot CLI today — see #66).
