...
> The Copilot CLI marketplace exposes a single installable plugin, `atv-starter-kit`, which bundles every ATV skill and agent (including `atv-skill-test-browser`). The granular `atv-skill-test-browser` entry in `.github/plugin/marketplace.json` is not individually installable through the Copilot CLI today.
