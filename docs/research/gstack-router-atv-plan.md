...
- **GATE 3 → Two-layer test split (accepted).** T5 becomes: (1) deterministic unit tests for the generator + config shim (real coverage, no LLM); (2) a small live-model smoke set accepting nondeterminism (judge the stable/dominant single route across a few runs — the router exposes one best route, not a ranked list; see `docs/atv-router.md`), including **no-match, collision, and PROACTIVE=false** cases. Stop asserting exact LLM routing picks.
